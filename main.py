...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_pool4 = tf.layers.conv2d_transpose(pool4_out_scaled, num_classes, 1, padding="same",
                                            kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                            kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    skip_pool4 = tf.add(output, conv_pool4)

    input_layer = tf.layers.conv2d_transpose(skip_pool4, num_classes, 4, strides=(2, 2), padding="same",
                                             kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    conv_pool3 = tf.layers.conv2d_transpose(pool3_out_scaled, num_classes, 1, padding="same",
                                            kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                            kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    tf.Print(output, [tf.shape(input_layer)])
